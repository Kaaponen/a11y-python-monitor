            ...
            alerts.append(
                {
                    ...
                    "level": (
                        "warning" if current.response_time_ms < 2000 else "critical"
                    ),
                ...
                with open(filename, "w", newline="") as f:
                    if self.metrics_history:
                        writer = csv.DictWriter(
                            f, fieldnames=self.metrics_history[0].to_dict().keys()
                        )
                        writer.writeheader()
...
def monitor_async_performance(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            logger.debug(
                f"Async function {func.__name__} executed in {execution_time:.2f}ms"
            )
            return result
