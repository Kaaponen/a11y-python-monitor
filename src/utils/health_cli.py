...
def status(output_format: str, verbose: bool):
    ...
    if output_format == 'json':
        ...
        click.echo(f"Status: {status_emoji.get(health_data['status'], '❓')} {health_data['status'].upper()}")
        click.echo(f"Uptime: {health_data['uptime_human']}")
        click.echo(f"Monitoring: {'🟢 Active' if health_data['system_resources']['monitoring_active'] else '🔴 Inactive'}")
...
def metrics(hours: int, output_format: str):
    ...
    if output_format == 'json':
        ...
        click.echo(f"  • Onnistumisprosentti: {metrics_data['success_rate_percent']:.1f}%")
        
        click.echo(f"\nSuorituskyky:")
        click.echo(f"  • Keskimääräinen kesto: {metrics_data['avg_duration_seconds']:.2f}s")
        click.echo(f"  • Rikkomuksia löydetty: {metrics_data['total_violations_found']}")
        click.echo(f"  • Keskimäärin per skannaus: {metrics_data['avg_violations_per_scan']:.1f}")
