...
class HealthMonitor:
        ...
        with self._lock:
            if scan_id in self.scan_metrics:
                ...
                if total_completed > 0:
                    total_duration = sum(
                        m.duration for m in self.scan_metrics.values()
                        if m.status == "completed" and m.duration
                    )
                    self.performance_stats['avg_scan_duration'] = total_duration / total_completed
    ...
    def fail_scan(self, scan_id: str, error_type: str):
        ...
        with self._lock:
            if scan_id in self.scan_metrics:
                ...
                logger.warning("Scan failed", extra={
                    "scan_id": scan_id,
                    "duration": metrics.duration,
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            recent_scans = [
                m for m in self.scan_metrics.values()
            ...
            success_rate = 0
            if total_scans > 0:
                success_rate = (self.performance_stats['successful_scans'] / total_scans) * 100
            
            return {
                'status': self._determine_overall_status(avg_cpu, avg_memory, success_rate),
                ...
                'performance': {
                    ...
                    'avg_scan_duration': round(self.performance_stats['avg_scan_duration'], 2),
                    'total_violations_found': self.performance_stats['total_violations']
                },
                'recent_activity': {
                    'scans_last_hour': len(recent_scans),
                    'active_scans': len([m for m in self.scan_metrics.values() if m.status == "running"])
                },
                'system_resources': {
    ...
    def _determine_overall_status(self, avg_cpu: float, avg_memory: float, success_rate: float) -> str:
    ...
    def get_metrics_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            avg_duration = total_duration / len(completed_scans) if completed_scans else 0
            
            total_violations = sum(m.violations_count for m in completed_scans)
            avg_violations = total_violations / len(completed_scans) if completed_scans else 0
            ...
            return {
                ...
                'success_rate_percent': (len(completed_scans) / len(period_scans)) * 100,
                'avg_duration_seconds': round(avg_duration, 2),
                'total_violations_found': total_violations,
                'avg_violations_per_scan': round(avg_violations, 1),
                'error_types': {
                    error_type: len([m for m in failed_scans if m.error_type == error_type])
                    for error_type in set(m.error_type for m in failed_scans if m.error_type)
                }
            }
    ...
    def cleanup_old_metrics(self, hours: int = 168):  # Default: 1 week
        ...
        with self._lock:
            old_scan_ids = [
                scan_id for scan_id, metrics in self.scan_metrics.items()
            ...
            if old_scan_ids:
                logger.info("Cleaned up old metrics", extra={
                    "removed_scans": len(old_scan_ids),
                    "cutoff_hours": hours
                })
