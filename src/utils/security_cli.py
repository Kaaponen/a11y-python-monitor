...
def rate_limit_status(client_id: str, output_format: str):
    ...
    if output_format == 'json':
        ...
        click.echo(f"Requests this hour: {status['requests_this_hour']}/{status['hour_limit']}")
        click.echo(f"Minute remaining: {status['minute_remaining']}")
        click.echo(f"Hour remaining: {status['hour_remaining']}")
        click.echo(f"Active scans: {status['active_scans']}/{status['max_concurrent_scans']}")
    ...
    async def audit_dependencies():
        ...
        if scan_result.scan_success:
            if scan_result.vulnerabilities:
                click.echo(f"❌ Found {len(scan_result.vulnerabilities)} vulnerabilities in {scan_result.vulnerable_packages} packages")
                
                # Show critical/high severity count
                critical_high = [v for v in scan_result.vulnerabilities if v.severity in ['CRITICAL', 'HIGH']]
                if critical_high:
                    click.echo(f"🚨 {len(critical_high)} critical/high severity vulnerabilities")
            else:
...
def format_scan_result_table(scan_result) -> str:
    ...
    for severity in ['CRITICAL', 'HIGH', 'MEDIUM', 'LOW', 'UNKNOWN']:
        if severity in severity_counts:
            ...
            emoji = {'CRITICAL': '🚨', 'HIGH': '❌', 'MEDIUM': '⚠️', 'LOW': '💛', 'UNKNOWN': '❓'}
