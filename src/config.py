...
class ReportsConfig:
    ...
    supported_formats: List[str] = field(
        default_factory=lambda: ["html", "json", "csv", "markdown"]
    )
...
class Config:
    ...
    def _load_from_environment(self):
        ...
        self.logging.structured = self._get_bool_env(
            "LOG_STRUCTURED", self.logging.structured
        )
        self.logging.file_path = os.getenv("LOG_FILE_PATH", self.logging.file_path)
        self.logging.include_console = self._get_bool_env(
            "LOG_INCLUDE_CONSOLE", self.logging.include_console
        )

        # Scanner
        self.scanner.timeout = self._get_int_env(
            "SCANNER_TIMEOUT", self.scanner.timeout
        )
        self.scanner.user_agent = os.getenv(
            "SCANNER_USER_AGENT", self.scanner.user_agent
        )
        self.scanner.max_concurrent_scans = self._get_int_env(
            "SCANNER_MAX_CONCURRENT", self.scanner.max_concurrent_scans
        )
        self.scanner.browser_type = os.getenv(
            "SCANNER_BROWSER_TYPE", self.scanner.browser_type
        )
        self.scanner.headless = self._get_bool_env(
            "SCANNER_HEADLESS", self.scanner.headless
        )

        # Reports
        self.reports.output_dir = os.getenv(
            "REPORTS_OUTPUT_DIR", self.reports.output_dir
        )
        self.reports.default_format = os.getenv(
            "REPORTS_DEFAULT_FORMAT", self.reports.default_format
        )
        self.reports.include_screenshots = self._get_bool_env(
            "REPORTS_INCLUDE_SCREENSHOTS", self.reports.include_screenshots
        )

        # Analysis
        self.analysis.openai_api_key = os.getenv(
            "OPENAI_API_KEY", self.analysis.openai_api_key
        )
        self.analysis.openai_model = os.getenv(
            "OPENAI_MODEL", self.analysis.openai_model
        )
        self.analysis.max_image_size_mb = self._get_int_env(
            "ANALYSIS_MAX_IMAGE_SIZE_MB", self.analysis.max_image_size_mb
        )

        # Performance
        self.performance.max_memory_mb = self._get_int_env(
            "PERFORMANCE_MAX_MEMORY_MB", self.performance.max_memory_mb
        )
        self.performance.metrics_enabled = self._get_bool_env(
            "PERFORMANCE_METRICS_ENABLED", self.performance.metrics_enabled
        )
    ...
    def _get_int_env(self, key: str, default: int) -> int:
        ...
        try:
            ...
            raise ConfigurationError(
                f"Invalid integer value for {key}: {os.getenv(key)}", key
            )
    ...
    def _get_float_env(self, key: str, default: float) -> float:
        ...
        try:
            ...
            raise ConfigurationError(
                f"Invalid float value for {key}: {os.getenv(key)}", key
            )
    ...
    def _validate(self):
        ...
        if self.logging.level.upper() not in valid_log_levels:
            raise ConfigurationError(
                f"Invalid log level: {self.logging.level}", "LOG_LEVEL"
            )

        # Validoi timeout
        if self.scanner.timeout <= 0:
            raise ConfigurationError(
                "Scanner timeout must be positive", "SCANNER_TIMEOUT"
            )

        if self.scanner.timeout > 300:  # 5 minuuttia maksimi
            raise ConfigurationError(
                "Scanner timeout cannot exceed 300 seconds", "SCANNER_TIMEOUT"
            )

        # Validoi samanaikaisten skannauksien määrä
        if self.scanner.max_concurrent_scans <= 0:
            raise ConfigurationError(
                "Max concurrent scans must be positive", "SCANNER_MAX_CONCURRENT"
            )

        if self.scanner.max_concurrent_scans > 100:
            raise ConfigurationError(
                "Max concurrent scans cannot exceed 100", "SCANNER_MAX_CONCURRENT"
            )
        ...
        if self.scanner.browser_type not in valid_browsers:
            raise ConfigurationError(
                f"Invalid browser type: {self.scanner.browser_type}",
                "SCANNER_BROWSER_TYPE",
            )

        # Validoi raporttiformaatti
        if self.reports.default_format not in self.reports.supported_formats:
            raise ConfigurationError(
                f"Default format not in supported formats", "REPORTS_DEFAULT_FORMAT"
            )

        # Validoi muistiraja
        if self.performance.max_memory_mb <= 0:
            raise ConfigurationError(
                "Max memory must be positive", "PERFORMANCE_MAX_MEMORY_MB"
            )
        ...
        return {
            ...
            "analysis": {
                **self.analysis.__dict__,
                "openai_api_key": (
                    "***masked***" if self.analysis.openai_api_key else None
                ),
            },
