...
from ..performance.fallback_cache import (
    cache_result,
...
async def run_axe(
    ...
    if use_cache:
        ...
        if cached_result:
            logger.info(
                ...
                extra={"url": url, "scan_id": scan_id, "cache_hit": True},
            )
    ...
    if not allowed:
        logger.warning(
            "Rate limit exceeded",
            extra={"client_id": client_id, "url": url, "reason": reason},
        )
    ...
    try:
        ...
        if not validation_result["valid"]:
            ...
            logger.error(
                "URL security validation failed",
                extra={
                    "url": url,
                    "scan_id": scan_id,
    ...
    logger.info(
        ...
        extra={"url": url, "scan_id": scan_id, "timeout": timeout},
    )
    ...
    try:
        async with async_playwright() as p:
            ...
            context = await browser.new_context(user_agent="SaavutettavuusSkanneri/1.0")
            ...
            try:
                ...
                if capture_screenshots and violation_count > 0:
                    await _capture_violation_screenshots(
                        page, axe_results, url, scan_id, screenshot_config or {}
                    )
                ...
                if use_cache and axe_results:
                    try:
                        ...
                        logger.debug(
                            "Scan result cached",
                            extra={"url": url, "scan_id": scan_id, "cache_ttl": 3600},
                        )
...
def format_results(results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.debug(
        ...
        extra={"formatted_violations": len(formatted["violations"])},
    )

    return formatted


async def _capture_violation_screenshots(
    page,
    ...
    try:
        ...
        logger.info(
            ...
            extra={"url": url, "scan_id": scan_id, "violations_count": len(violations)},
        )

        # Capture page overview first
        if screenshot_config.get("capture_overview", True):
            overview_info = await screenshot_capture.capture_page_overview(
                page, violations
            )
            if overview_info:
        ...
        for violation in violations:
            ...
            for node_index, node in enumerate(violation.get("nodes", [])):
                ...
                for target in node.get("target", []):
                    try:
                        logger.info(f"Capturing screenshot for target: {target}")
                        screenshot_info = (
                            await screenshot_capture.capture_violation_element(
                                page, target, violation, node_index
                            )
                        )

                        logger.info(f"Screenshot result: {screenshot_info is not None}")
                        if screenshot_info:
                            logger.info(f"Screenshot keys: {screenshot_info.keys()}")
                            node_screenshots.append(screenshot_info)
                            screenshot_count += 1

                            # Add base64 data directly to node for UI display
                            if "base64_data" in screenshot_info and not node.get(
                                "screenshot"
                            ):
                                node["screenshot"] = screenshot_info["base64_data"]
                                logger.info("Added base64 screenshot to node")

                            # Limit screenshots per violation to prevent overflow
                            if len(node_screenshots) >= screenshot_config.get(
                                "max_screenshots_per_node", 3
                            ):
                                break
                        else:
                            logger.warning(
                                f"No screenshot captured for target: {target}"
                            )

                    except Exception as e:
                        logger.warning(
                            "Failed to capture screenshot for target",
                            extra={
                                "target": target,
        ...
        logger.error(
            "Failed to capture violation screenshots",
            extra={"url": url, "scan_id": scan_id, "error": str(e)},
            exc_info=True,
        )

        # Add error info to results
        axe_results["screenshot_error"] = {"error": str(e), "timestamp": time.time()}
