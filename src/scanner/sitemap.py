...
async def get_urls_from_sitemap(
    sitemap_url: str, prefix_filter: Optional[str] = None, use_cache: bool = True
) -> List[str]:
    """
    ...
    try:
        ...
        if use_cache and urls:
            try:
                ...
                logger.warning(
                    "Failed to cache sitemap URLs",
                    extra={"sitemap_url": sitemap_url, "cache_error": str(cache_error)},
                )
        ...
        logger.error(
            ...
            extra={"sitemap_url": sitemap_url, "error": str(e)},
        )
        raise


async def _parse_sitemap(
    sitemap_url: str, prefix_filter: Optional[str] = None
) -> List[str]:
    """
    ...
    try:
            ...
            if sitemap_tasks:
                ...
                for result in results:
                    if isinstance(result, list):
                        ...
                        logger.warning(
                            "Sitemap parsing failed", extra={"error": str(result)}
                        )
        else:
            # Process regular sitemap
            logger.debug(
                "Processing regular sitemap", extra={"sitemap_url": sitemap_url}
            )
        ...
        logger.error(
            "XML parsing failed", extra={"sitemap_url": sitemap_url, "error": str(e)}
        )
        raise ScannerError(f"Invalid XML in sitemap: {e}")

    except Exception as e:
        logger.error(
            "Sitemap parsing error", extra={"sitemap_url": sitemap_url, "error": str(e)}
        )
        raise NetworkError(f"Failed to process sitemap: {e}", sitemap_url)


# Legacy sync function for backwards compatibility
def get_urls_from_sitemap_sync(
    sitemap_url: str, prefix_filter: Optional[str] = None
) -> List[str]:
    """
    ...
    """
    logger.warning(
        "Using legacy sync sitemap function", extra={"sitemap_url": sitemap_url}
    )

    return asyncio.run(
        get_urls_from_sitemap(sitemap_url, prefix_filter, use_cache=False)
    )
