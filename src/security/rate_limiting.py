...
class RateLimiter:
    ...
    def _create_entry(self) -> RateLimitEntry:
        ...
        return RateLimitEntry(
            requests=deque(), last_request=0, total_requests=0, blocked_requests=0
        )

    def check_rate_limit(
        self, client_id: str, operation: str = "scan"
        ...
        with self._lock:
            ...
            if entry.blocked_until and current_time < entry.blocked_until:
                ...
                reason = (
                    f"Rate limit exceeded. Blocked for {remaining:.0f} more seconds"
                )
                logger.warning(
            ...
            if entry.blocked_until and current_time >= entry.blocked_until:
                ...
                logger.info("Rate limit block expired", extra={"client_id": client_id})
            ...
            if operation == "scan":
                ...
                expired_scans = [
                    cid
                    for cid, start_time in self.active_scans.items()
            ...
            minute_requests = sum(
                1 for req_time in entry.requests if req_time > minute_ago
            )
            ...
            if self._is_burst_attack(entry, current_time):
                ...
                logger.warning(
                    ...
                    extra={"client_id": client_id, "operation": operation},
                )
                return False, reason
    ...
    def _apply_rate_limit_penalty(
        self, entry: RateLimitEntry, current_time: float, reason: str
    ):
    ...
    def get_rate_limit_status(self, client_id: str) -> Dict[str, any]:
        ...
        with self._lock:
            ...
            status = {
                ...
                "is_blocked": entry.blocked_until
                and current_time < entry.blocked_until,
                "blocked_until": entry.blocked_until,
