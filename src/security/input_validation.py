...
class SecurityValidator:
    ...
    def validate_url(self, url: str, allow_private: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(url) > MAX_URL_LENGTH:
                result['security_issues'].append(f"URL too long: {len(url)} > {MAX_URL_LENGTH}")
                logger.warning("URL length validation failed", extra={
                    "url_length": len(url),
            ...
            if self.suspicious_pattern.search(url):
                ...
                logger.warning("Suspicious patterns in URL", extra={
                    "url": url,
            ...
            if parsed.scheme.lower() not in ALLOWED_PROTOCOLS:
                ...
                logger.warning("Invalid protocol detected", extra={
            ...
            if hostname in BLOCKED_DOMAINS:
                ...
                logger.warning("Blocked domain access attempt", extra={
                    "hostname": hostname,
            ...
            try:
                ...
                if not allow_private and self._is_private_ip(ip):
                    result['security_issues'].append(f"Private IP address not allowed: {hostname}")
                    logger.warning("Private IP access attempt", extra={
                        "ip": str(ip),
            ...
            if parsed.port:
                ...
                if parsed.port in dangerous_ports:
                    result['warnings'].append(f"Potentially dangerous port: {parsed.port}")
                    logger.info("Potentially dangerous port", extra={
                        "port": parsed.port,
                        "url": url
                    })
            
            result['valid'] = True
            logger.debug("URL validation successful", extra={
                "url": url,
                "hostname": hostname,
    ...
    def sanitize_filename(self, filename: str) -> str:
        ...
        if len(filename) > MAX_FILENAME_LENGTH:
            raise SecurityError(f"Filename too long: {len(filename)} > {MAX_FILENAME_LENGTH}")
        
        # Check for dangerous extensions
        lower_filename = filename.lower()
        for ext in DANGEROUS_EXTENSIONS:
            if lower_filename.endswith(ext):
                logger.warning("Dangerous file extension detected", extra={
        ...
        if self.suspicious_pattern.search(filename):
            logger.warning("Suspicious patterns in filename", extra={
                "original_filename": filename,
        ...
        if name_without_ext in reserved_names:
            ...
            logger.info("Reserved filename sanitized", extra={
    ...
    def validate_report_format(self, format_name: str) -> bool:
        ...
        if format_name.lower() not in allowed_formats:
            logger.warning("Invalid report format requested", extra={
                "format": format_name,
    ...
    def validate_sitemap_urls(self, urls: List[str], max_urls: int = 1000) -> List[str]:
        ...
        if len(urls) > max_urls:
            logger.warning("Too many URLs in sitemap", extra={
                "url_count": len(urls),
                "max_allowed": max_urls
            })
        ...
        for url in urls:
            try:
                ...
                logger.warning("URL failed validation in sitemap", extra={
                    "url": url,
                    "error": str(e)
                })
        ...
        logger.info("Sitemap URLs validated", extra={
            "total_urls": len(urls),
