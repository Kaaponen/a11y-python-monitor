...
with st.sidebar:
    ...
    enable_screenshots = st.checkbox(
        "Ota kuvakaappauksia virheellisistä elementeistä", value=False
    )

    if enable_screenshots:
        highlight_violations = st.checkbox("Korosta virheelliset elementit", value=True)
        element_padding = st.slider(
            "Elementin padding (px)", min_value=0, max_value=50, value=20
        )
        max_screenshots = st.slider(
            "Max kuvakaappauksia per virhe", min_value=1, max_value=10, value=3
        )
    ...
    scan_timeout = st.slider(
        "Skannauksen timeout (s)", min_value=10, max_value=120, value=30
    )
...
if mode == "Yksittäinen URL":
    ...
    sitemap_url = st.text_input("Sitemap.xml osoite", "https://example.com/sitemap.xml")
...
if urls and st.button("🚀 Skannaa saavutettavuus"):
    ...
    try:
                    ...
                    for node in v["nodes"]:
                        ...
                        if "screenshot" in node:
                            ...
                            try:
                                ...
                                st.image(
                                    screenshot_data,
                                    caption=f"Virhe: {v['id']}",
                                    use_column_width=True,
                                )
                            except Exception as e:
                                st.warning(
                                    f"Kuvakaappauksen näyttäminen epäonnistui: {e}"
                                )
        ...
        st.download_button(
            "📄 Markdown", data=open(md_path, "rb"), file_name=os.path.basename(md_path)
        )
        st.download_button(
            "🌐 HTML", data=open(html_path, "rb"), file_name=os.path.basename(html_path)
        )
        st.download_button(
            "📊 CSV", data=open(csv_path, "rb"), file_name=os.path.basename(csv_path)
        )
        st.download_button(
            "🧾 JSON", data=open(json_path, "rb"), file_name=os.path.basename(json_path)
        )

    except Exception as e:
        st.error(f"Virhe skannauksessa: {str(e)}")
        st.write(
            "Tarkista että URL:t ovat oikeassa muodossa (esim. https://example.com)"
        )
