        ...
        with patch('src.reports.reporter.os.makedirs'):
            with patch('src.reports.reporter.datetime') as mock_datetime:
                ...
                with patch('builtins.open', mock_open()) as mock_file:
                    ...
                    # Tarkista MD sisältö
                    md_calls = [call for call in mock_file().write.call_args_list if 'Accessibility Report' in str(call)]
        ...
        with patch('src.reports.reporter.os.makedirs'):
            with patch('src.reports.reporter.datetime') as mock_datetime:
                ...
                with patch('builtins.open', mock_open()) as mock_file:
                    ...
                    assert "Elements must have sufficient color contrast" in written_content
        ...
        with patch('src.reports.csv_export.os.makedirs'):
            with patch('src.reports.csv_export.datetime') as mock_datetime:
                ...
                with patch('builtins.open', mock_open()) as mock_file:
                    ...
                    # Tarkista polku
                    expected_path = os.path.join(str(tmp_path), "report_20231024_120000.csv")
...
class TestJSONExport:
    ...
    def test_save_json_basic(self, tmp_path):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert loaded_data["https://example.com"]["violations"][0]["id"] == "test-rule"
        ...
        with patch('src.reports.json_export.os.makedirs'):
            with patch('src.reports.json_export.datetime') as mock_datetime:
                ...
                with patch('builtins.open', mock_open()):
                    ...
                    expected_path = os.path.join("test_output", "report_20231024_120000.json")
